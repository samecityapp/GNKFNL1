...
export default async function HotelDetailPage({ params }: Props) {
  ...
  return (
    <>
      ...
      {/* Mobil Görünüm - Airbnb Tarzı */}
      <div className="sm:hidden">
        {/* Fotoğraf Galerisi - Header'dan hemen sonra */}
        <div className="relative">
          ...
          {/* Geri Butonu - Foto Üzerinde */}
          <div className="absolute top-4 left-4 z-10">
            <BackButton variant="overlay" />
          </div>
        </div>

        {/* Otel Bilgileri Container */}
        <div className="px-4 py-6">
          {/* Otel İsmi - Ortalı */}
          <h1 className="text-2xl font-bold text-gray-900 text-center mb-4">{hotel.name}</h1>

          {/* Gecelik Fiyat - Ortalı */}
          <div className="text-center mb-6">
            <p className="text-sm text-gray-600 mb-1">Gecelik Fiyat</p>
            <p className="text-3xl font-bold text-gray-900">{hotel.price.toLocaleString('tr-TR')} ₺</p>
          </div>

          {/* 3 Bölüm: Konum, Harita, Puan */}
          <div className="grid grid-cols-3 gap-3 mb-6">
            {/* Sol - Konum */}
            <div className="bg-white border-2 border-gray-200 rounded-xl p-3 flex flex-col items-center justify-center text-center">
              <MapPin className="w-6 h-6 text-gray-700 mb-1.5" />
              <p className="text-xs font-semibold text-gray-900 leading-tight">{hotel.location}</p>
            </div>

            {/* Orta - Google Haritalar */}
            {hotel.google_maps_url && (
              <a
                href={hotel.google_maps_url}
                target="_blank"
                rel="noopener noreferrer"
                className="bg-green-50 border-2 border-green-600 rounded-xl p-3 flex flex-col items-center justify-center text-center hover:bg-green-100 transition-colors active:scale-95"
              >
                <svg className="w-6 h-6 mb-1.5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <p className="text-xs font-bold text-green-700 leading-tight">Haritada<br/>Gör</p>
              </a>
            )}

            {/* Sağ - Puan */}
            <div className="bg-blue-50 border-2 border-blue-600 rounded-xl p-3 flex flex-col items-center justify-center text-center">
              <p className="text-[10px] font-medium text-gray-600 mb-0.5">GNK Puan</p>
              <p className="text-2xl font-bold text-blue-600 leading-tight">{rating.score}</p>
            </div>
          </div>

          {/* Otele Git ve Instagram Butonları */}
          <div className="flex flex-col gap-3 mb-6">
            {hotel.website_url && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 bg-gray-900 text-white font-semibold py-3 px-6 rounded-xl transition-colors active:scale-95"
              >
                <span>Otele Git</span>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
              </a>
            ...
            {hotel.instagram_url && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 bg-white border-2 border-gray-900 text-gray-900 font-semibold py-3 px-6 rounded-xl transition-colors active:scale-95"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
                <span>Instagram</span>
              </a>
