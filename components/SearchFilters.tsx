  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4">
      ...
      <div className="mt-6 space-y-3">
        ...
        {priceTags.length > 0 && (
          <div className="flex items-center justify-center gap-2 flex-wrap">
            {priceTags.slice(0, 5).map(pt => (
              <Link
                key={pt.id}
                href={`/search?q=${pt.slug}`}
                className="px-3.5 py-1.5 bg-gray-100 hover:bg-gray-900 text-gray-700 hover:text-white rounded-full text-xs font-medium transition-all duration-200 border border-transparent hover:border-gray-900"
              >
                {pt.label}
              </Link>
            ))}
